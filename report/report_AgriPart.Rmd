...
|:---------------------------:	|:----------------:	|
| plain python (numpy) 	| 8.27 s ± 216 ms  	|
| numba 	| 8.04 s ± 259 ms 	|
| cpp | 316 ms ± 6.25 ms|

While providing same results, JIT compilation (numba) only improve the algorithm a little, from 8.27 s ± 216 ms per loop to 8.04 s ± 259 ms per loop. However, cpp provides a great efficiency, making the time improved to 316 ms ± 6.25 ms per loop.
...
We can that the distributin of SGHMC gives similar result as the distribution of simulated data. Theoretically, as $\epsilon$ close to 0, the SGHMC maintain the innvariance of the Hamiltonian function.
